// based on default_libpd_render.cpp at 554aa0a1f63207aea6a1111dcfcf6028ca91a710.
// Connects to a VL530LX distance sensor and sends its readings to Pd
// check for modifications "BELA_LIBPD_VL530LX"
...
#ifdef BELA_LIBPD_VL530LX
#include <VL53L0X.hpp>
static VL53L0X gVl53;
static uint16_t gVl53Distance;
void readVl53(void*)
{
	gVl53.setTimeout(200);
	while(!Bela_stopRequested())
	{
		gVl53Distance = gVl53.readRangeSingleMillimeters();
		if (gVl53.timeoutOccurred()) {
			fprintf(stderr, "VL530X TIMEOUT OCCURRED\n");
		}
		// make sure it reads at least as fast as it sends to Pd in render()
		usleep(50000);
	}
}
#endif // BELA_LIBPD_VL530LX
...
#ifdef BELA_LIBPD_VL530LX
	gVl53.initialize();
	Bela_runAuxiliaryTask(readVl53);
#endif // BELA_LIBPD_VL530LX
	// Check Pd's version
...
#ifdef BELA_LIBPD_VL530LX
	{
		static uint64_t vl53LastSent = 0;
		// send every 20 ms. Make sure this is at least as fast as the
		// loop in readVl53
		static uint64_t vl53Period = 0.02 * context->audioSampleRate;
		if(context->audioFramesElapsed > vl53LastSent + vl53Period)
		{
			vl53LastSent = context->audioFramesElapsed;
			libpd_float("vl53l0x", gVl53Distance);
		}
	}
#endif // BELA_LIBPD_VL530LX
